...
public class ExtendedPersistentEntitySectionManager<T extends EntityAccess> extends PersistentEntitySectionManager<T> implements AutoCloseable {
    ...
    public ExtendedPersistentEntitySectionManager(Class<T> entityClass, LevelCallback<T> callbacks, EntityPersistentStorage<T> permanentStorage) {
        super(entityClass, callbacks, permanentStorage);
        this.visibleEntityStorage = new EntityLookup<>();
        this.sectionStorage = new ExtendedEntitySectionStorage<>(entityClass, chunkVisibility);
        ...
        this.entityGetter = new ExtendedLevelEntityGetterAdapter<>(this.visibleEntityStorage, this.sectionStorage);
    }

    void removeSectionIfEmpty(CoordinateKey sectionKey, EntitySection<T> section) {
        if (section.isEmpty()) {
    ...
    private boolean addEntityWithoutEvent(T p_157539_, boolean p_157540_) {
        if (!this.addEntityUuid(p_157539_)) {
            ...
            BlockPos pos = p_157539_.blockPosition();

            CoordinateKey sectionKey = new CoordinateKey(
            ...
            EntitySection<T> entitysection =
                    this.sectionStorage.getOrCreateSection(sectionKey);
            entitysection.add(p_157539_);
            p_157539_.setLevelCallback(new ExtendedPersistentEntitySectionManager.Callback(p_157539_, sectionKey, entitysection));
            if (!p_157540_) {
    ...
    public void updateChunkStatus(ChunkPos pos, Visibility p_visibility) {
        ...
        this.sectionStorage.getExistingSectionsInChunk(i).forEach(entitySection -> {
            Visibility visibility = entitySection.updateChunkStatus(p_visibility);
            if (visibility == null) visibility = Visibility.HIDDEN;
            ...
            if (flag2 && !flag3) {
                entitySection.getEntities().filter(p_157603_ -> !p_157603_.isAlwaysTicking()).forEach(this::stopTicking);
            }

            if (flag && !flag1) {
                entitySection.getEntities().filter(p_157601_ -> !p_157601_.isAlwaysTicking()).forEach(this::stopTracking);
            } else if (!flag && flag1) {
                entitySection.getEntities().filter(p_157599_ -> !p_157599_.isAlwaysTicking()).forEach(this::startTracking);
            }

            if (!flag2 && flag3) {
                entitySection.getEntities().filter(p_157597_ -> !p_157597_.isAlwaysTicking()).forEach(this::startTicking);
            }
        });
    }

    private void ensureChunkQueuedForLoad(long chunkPosValue) {
        ChunkLoadStatus persistententitysectionmanager$chunkloadstatus = this.chunkLoadStatuses.get(chunkPosValue);
    ...
    private boolean storeChunkSections(long chunkPosValue, Consumer<T> entityAction) {
        ...
        if (persistententitysectionmanager$chunkloadstatus == ChunkLoadStatus.PENDING) {
            ...
            if (list.isEmpty()) {
                ...
                return true;
            } else if (persistententitysectionmanager$chunkloadstatus == ChunkLoadStatus.FRESH) {
                this.requestChunkLoad(chunkPosValue);
                return false;
            } else {
    ...
    private void requestChunkLoad(long chunkPosValue) {
        this.chunkLoadStatuses.put(chunkPosValue, ChunkLoadStatus.PENDING);
    ...
    private LongSet getAllChunksToSave() {
        ...
        for (Long2ObjectMap.Entry<ChunkLoadStatus> entry : Long2ObjectMaps.fastIterable(this.chunkLoadStatuses)) {
    ...
    public void dumpSections(Writer writer) throws IOException {
        ...
        this.sectionStorage
                ...
                .forEach(
                        p_157517_ -> {
                            ChunkLoadStatus persistententitysectionmanager$chunkloadstatus = this.chunkLoadStatuses.get(p_157517_);
                            this.sectionStorage
    ...
    class Callback implements EntityInLevelCallback {
        ...
        Callback(T entity, CoordinateKey currentSectionKey, EntitySection<T> currentSection) {
            this.entity = entity;
        ...
        public void onMove() {
            ...
            if (newSectionKey != this.currentSectionKey) {
                ...
                if (!this.currentSection.remove(this.entity)) {
                    ExtendedPersistentEntitySectionManager.LOGGER
                            .warn("Entity {} wasn't found in section {} (moving to {})", this.entity, this.currentSectionKey, newSectionKey);
                }

                ExtendedPersistentEntitySectionManager.this.removeSectionIfEmpty(this.currentSectionKey, this.currentSection);
                EntitySection<T> entitysection = ExtendedPersistentEntitySectionManager.this.sectionStorage.getOrCreateSection(newSectionKey);
                entitysection.add(this.entity);
                CoordinateKey oldSectionKey = currentSectionKey;
                this.currentSection = entitysection;
                this.currentSectionKey = newSectionKey;
                this.updateStatus(visibility, entitysection.getStatus());
//                if (this.realEntity != null) net.neoforged.neoforge.common.CommonHooks.onEntityEnterSection(this.realEntity, oldSectionKey, newSectionKey); // disable this neoforge event because it expects longs
            }
        }

        private void updateStatus(Visibility oldVisibility, Visibility newVisibility) {
            Visibility visibility = ExtendedPersistentEntitySectionManager.getEffectiveStatus(this.entity, oldVisibility);
            Visibility visibility1 = ExtendedPersistentEntitySectionManager.getEffectiveStatus(this.entity, newVisibility);
            if (visibility == visibility1) {
                if (visibility1.isAccessible()) {
                    ExtendedPersistentEntitySectionManager.this.callbacks.onSectionChange(this.entity);
                }
            } else {
                boolean flag = visibility.isAccessible();
                boolean flag1 = visibility1.isAccessible();
                if (flag && !flag1) {
                    ExtendedPersistentEntitySectionManager.this.stopTracking(this.entity);
                } else if (!flag && flag1) {
                    ExtendedPersistentEntitySectionManager.this.startTracking(this.entity);
                }

                boolean flag2 = visibility.isTicking();
                boolean flag3 = visibility1.isTicking();
                if (flag2 && !flag3) {
                    ExtendedPersistentEntitySectionManager.this.stopTicking(this.entity);
                } else if (!flag2 && flag3) {
                    ExtendedPersistentEntitySectionManager.this.startTicking(this.entity);
                }
        ...
        public void onRemove(Entity.RemovalReason reason) {
            ...
            Visibility visibility = ExtendedPersistentEntitySectionManager.getEffectiveStatus(this.entity, this.currentSection.getStatus());
            if (visibility.isTicking()) {
                ExtendedPersistentEntitySectionManager.this.stopTicking(this.entity);
            }
            ...
            if (reason.shouldDestroy()) {
                ExtendedPersistentEntitySectionManager.this.callbacks.onDestroyed(this.entity);
            }

            ExtendedPersistentEntitySectionManager.this.knownUuids.remove(this.entity.getUUID());
